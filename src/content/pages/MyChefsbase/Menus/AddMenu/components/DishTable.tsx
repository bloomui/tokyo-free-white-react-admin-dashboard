...
import { CourseToDishesInput } from "src/globalTypes";
import { dishForCourse } from "..";
import { units } from "../../../Ingredients/ingredientDialogs/UpdateIngredientDialog";
import { dishForCourseid } from "../../menuDialog/UpdateMenu";
import { dishRowsPerPage, useSearchDishesQuery } from "../api";
import { dishes_dishes } from "../types/dishes";

export  const TableDishDataId = ({
    courses,
  setDishes
}: {
    courses: CourseToDishesInput[];
    setDishes: (selected: dishForCourseid) => void
}) => {
    ...
    onKeyPress= {(e) => {
      ...
      onChange={(e) => setName(e.target.value)}    />
        ...
        {data.dishes.map((dish) => (
          ...
          data={dish}
          courses={courses}
          setDish={(a: dishForCourseid) => setDishes(a)}/>
        ))}
        </Table>
        <TablePagination
...
const Rowid = ({courses, data, setDish}: {courses: CourseToDishesInput[], data: dishes_dishes, setDish: (a) => void}) => {

  const formState: dishForCourseid = {
  dishid: data.id,
  courseid: '',
  coursetype: '',
  dishname: data.name
      ...
      return (
        ...
        <Dialog open={open} onClose={() => setOpen(false)}>
              ...
              {courses.map((courseid) => (
                <MenuItem key={courseid.courseid} value={courseid.courseid}>{courseid.coursetype}</MenuItem>
              ))}
            </FormikSelect>
                        </TableCell>
                        <TableCell>
